...
//Extremos
    ...
    public envelope_larc03(Context myContext,
...
//Procuramos as propriedades da lâmina
    ...
    int global_larc03_numero_circulos=10;
    double global_larc03_alpha_0=0.0;
    int   global_larc03_numero_elementos_circulos=10;
    ...
    public double[][] fazer_grafica(Double angulo, Double tau_xy,
                        int larc03_numero_circulos,
                        double larc03_alpha_0,
                        int   larc03_numero_elementos_circulos,
                        double larc03_TAU23,
                        double larc03_Y_T_is,
                        double larc03_S_L_is
    ) {

        global_larc03_numero_circulos=larc03_numero_circulos;
        global_larc03_alpha_0=larc03_alpha_0;
        global_larc03_numero_elementos_circulos=larc03_numero_elementos_circulos;
        global_larc03_TAU23=larc03_TAU23;
        global_larc03_Y_T_is=larc03_Y_T_is;
        global_larc03_S_L_is=larc03_S_L_is;
...
//Caso do Termo T
        ...
        if(sigma_2<0) {
            if(sigma_2<=sigma_1) {
                ...
                Double eta_L = 0.0;
                Double previo=(TAU12 * Math.cos(alpha_0 * 2)) / (SIGMA_C_2 * Math.pow(Math.cos(alpha_0), 2));
                if(previo>0)
                {
                    eta_L = previo;
                }
                ...
                double termo_L = Math.pow(tau_eff_L / TAU12, 2);
			//Caso do Termo L

                Double eta_T = 0.0;
                previo=-1 / (Math.tan(2 * alpha_0));
                if(previo>0)
                {
                    eta_T = previo;
                }
...
double TAU23=global_larc03_TAU23;
                double termo_T = Math.pow(tau_eff_T / TAU23, 2);
...
// Matrix Cracking
        ...
        if(sigma_2>=0)
        {
            double NU21=f_NU12*(f_LARC03_E2/f_LARC03_E1);
            double lambda_22=2*((1/f_LARC03_E2)-(Math.pow(NU21,2)/f_LARC03_E1));
...
double Y_T_is=global_larc03_Y_T_is;//1.12*Math.sqrt(2)*SIGMA_T_2;//global_larc03_Y_T_is;//
double S_L_is=global_larc03_S_L_is;//Math.sqrt(2)*TAU12;//global_larc03_S_L_is;//

            /*
            Log.d("Nov06","PURO global_larc03_TAU23  "+global_larc03_TAU23);

            Log.d("Nov06","PURO TAU12  "+TAU12);
            Log.d("Nov06","PURO global_larc03_S_L_is  "+global_larc03_S_L_is);
            Log.d("Nov06","PURO calculado  "+Math.sqrt(2)*TAU12);
...
//Modo 3 , TRAÇÂO na fibra:
    ...
    if(sigma_1>=0)
    {
        double numerador_if_larc_03_tracao_fibra=((sigma_1/f_LARC03_E1)-(sigma_2/f_LARC03_E2)*f_NU12);
        if_larc_03_tracao_fibra=numerador_if_larc_03_tracao_fibra/f_LARC03_EPSILON_T_1;
    }
...
//	Modo 4, Compressão na Fibra, Depende do valor de sigma_2_m
    ...
    if(sigma_1<0)
    {
        if(sigma_2_m<0)
        {
            double S_L_is=global_larc03_S_L_is;//Math.sqrt(2)*TAU12;
            double alpha_0=global_larc03_alpha_0;//53.2*(Math.PI/180);
            double eta_L=(TAU12*Math.cos(alpha_0*2))/(SIGMA_C_2*Math.pow(Math.cos(alpha_0),2));
...
//	Modo 5, Compressão na Fibra, Depende do valor de fi
    ...
    if(sigma_1<0)
    {
        if(sigma_2_m>=0)
        {
			...
			//caso lâmina grossa
			// estes valores de SIGMA_T_2 podem se usar como uma boa aproximação
            double Y_T_is=global_larc03_Y_T_is;//1.12*Math.sqrt(2)*SIGMA_T_2;
            g=(lambda_22/lambda_44)*Math.pow((Y_T_is/S_L_is),2);
...
//#Caso do Termo T_m
            double alpha_0=53.2*(Math.PI/180);
double alpha=global_larc03_alpha_0;//alpha_0;
            double eta_L=0.0;
            double tau_eff_L=Math.cos(alpha)*(Math.abs(tau_12)+eta_L*sigma_2*Math.cos(alpha));
            double termo_L=Math.pow(tau_eff_L/TAU12,2);
		//#	Caso do Termo L_m
double TAU23=global_larc03_TAU23;//TAU12;
    ...
    IFs[2] = if_larc_03_compressao_matriz_larc0301; //
...
public double[][] fazer_graficaANTIGO(Double angulo, Double tau_xy)
{
