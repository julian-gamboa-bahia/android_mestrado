 ...
 * //marco29
 * Este envelope é muito similar ao Tsai-Wu.
 *
 *
 */

public class envelope_tsai_wu {
    ...
    public envelope_tsai_wu(Context myContext, String lamina_usada,
...
* */
    ...
    public double[][]  fazer_grafica(Double angulo, Double tau_xy,Double biaxial_experimental)
    {
...
//PRE Coeficientes
        ...
        Double is1q=(1/f_SIGMA_T_1)*(1/f_SIGMA_T_1);
        Double is2q=(1/f_SIGMA_T_2)*(1/f_SIGMA_T_2);
        ...
        Double F12=(1/(2*biaxial_experimental*biaxial_experimental))*F12_sem_normalizar;
...
* */
        double fatorRotar_a=
                2*F12*Math.pow(cos,2)*Math.pow(sin,2) //2*F12*Math.pow(cos,2)*Math.pow(sin,2)
                + Math.pow(cos,4)*is1q //+ Math.pow(cos,4)*is1q
                + Math.pow(cos,2)*Math.pow(sin,2)*t12 //+ Math.pow(cos,2)*Math.pow(sin,2)*t12
                + is2q*Math.pow(sin,4);  //+ is2q*Math.pow(sin,4);
/*
Original:
fatorRotar_c=
+ 2*F12*Math.pow(cos,2)*Math.pow(sin,2)
...
double 	fatorRotar_c=
        + 2*F12*Math.pow(cos,2)*Math.pow(sin,2)     //+ 2*F12*Math.pow(cos,2)*Math.pow(sin,2)
        + Math.pow(cos,4)*is2q                      //+ Math.pow(cos,4)*is2q
        + Math.pow(cos,2)*Math.pow(sin,2)*t12       //+ Math.pow(cos,2)*Math.pow(sin,2)*t12
        + is1q*Math.pow(sin,4);                     //+ is1q*Math.pow(sin,4);
...
* */
        double fatorRotar_b=
        2*F12* Math.pow(cos,4) //2*F12* Math.pow(cos,4)
        + 2*F12*Math.pow(sin,4) //+ 2*F12*Math.pow(sin,4)
        + 2*Math.pow(cos,2)*is1q*Math.pow(sin,2) //+ 2*Math.pow(cos,2)*is1q*Math.pow(sin,2)
        + 2*Math.pow(cos,2)*is2q*Math.pow(sin,2) //+ 2*Math.pow(cos,2)*is2q*Math.pow(sin,2)
        - 2*Math.pow(cos,2)*Math.pow(sin,2)*t12; //- 2*Math.pow(cos,2)*Math.pow(sin,2)*t12
/*
fatorRotar_d=
...
* */
            ...
            double fatorRotar_d=
                    -4*F12*Math.pow(cos,3)*sin*tauXY //-4*F12*Math.pow(cos,3)*sin*tauXY
                    + 4*F12*cos*Math.pow(sin,3)*tauXY //+ 4*F12*cos*Math.pow(sin,3)*tauXY
                    + 4*Math.pow(cos,3)*is1q*sin*tauXY //+ 4*Math.pow(cos,3)*is1q*sin*tauXY
                    - 2*Math.pow(cos,3)*sin*t12*tauXY //- 2*Math.pow(cos,3)*sin*t12*tauXY
                    + Math.pow(cos,2)*is1               //+ Math.pow(cos,2)*is1
                    - 4*cos*is2q*Math.pow(sin,3)*tauXY //- 4*cos*is2q*Math.pow(sin,3)*tauXY
                    + 2*cos*Math.pow(sin,3)*t12*tauXY //+ 2*cos*Math.pow(sin,3)*t12*tauXY
                    + is2*Math.pow(sin,2);          //+ is2*Math.pow(sin,2);
...
// no caso isotropico Poupamos calculo e fazemos apenas o estudo polar:
        ...
        double raioMaior=Math.sqrt((2*(a*f*f+c*d*d+g*b*b-2*b*d*f-a*c*g))/((b*b-a*c)*(Math.sqrt(Math.pow((a-c),2)+4*b*b)-(a+c))));
        double raioMenor=Math.sqrt((2*(a*f*f+c*d*d+g*b*b-2*b*d*f-a*c*g))/((b*b-a*c)*(-Math.sqrt(Math.pow((a-c),2)+4*b*b)-(a+c))));
...
///////////////////////////////
        ...
        {
            Log.d("O29WUWU","  "+i+"  "+pares_pontos[i][0]);
        }
*/
